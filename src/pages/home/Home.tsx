...
function Home() {
    ...
    function SamplePrevArrow(props: any) {
        ...
        return (
            <div
                className={className}
                style={{ ...style, position: "absolute", left: '0', zIndex: '2' }}
                onClick={onClick}
            />
        );
    }
    ...
    return (
        <Box className="slider-container" sx={{ width: '100%', margin: '0 auto', height: '788px', position: 'relative' }}>
            <Slider ref={arrowRef} {...settings}>
                {slides.map((slide, index) => (
                    ...
                    >
                        ...
                        >
                            <Typography variant="h3" sx={{ marginBottom: '16px', fontWeight: 'bold', color: 'var(--softWhite-color)', fontSize: '32px' }}>
                                {slide.title}
                            </Typography>
            ...
            <IconButton onClick={() => arrowRef.current.slickNext()} sx={{ position: 'absolute', right: 0, top: '50%', transform: 'translateY(-50%)' }}>
                <ChevronRightRoundedIcon sx={{ fontSize: '64px', color: 'var(--softWhite-color)' }} />
            </IconButton>

            <IconButton onClick={() => arrowRef.current.slickPrev()} sx={{ position: 'absolute', left: 0, top: '50%', transform: 'translateY(-50%)' }}>
